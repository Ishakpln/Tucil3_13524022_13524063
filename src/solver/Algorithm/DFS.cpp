...
DFS::DFS(const Board& board) : Solver(board) {}

std::string DFS::getStateKey(Node node) const {
    return std::to_string(node.position.x) + "," +
           std::to_string(node.position.y) + "," +
           std::to_string(node.targetIndex);
}

Result DFS::constructPath(
...
Result DFS::solve() {
    ...
    while (!processStack.empty()) {
        Node candidateNode = processStack.top();
        processStack.pop();
...
void DFS::saveSolution(const std::string& outputPath, const Result& result) {
