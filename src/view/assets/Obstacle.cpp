...
void Obstacle::drawAt(float boardX, float boardY, float tileSize, int row, int col,
                      int drawWidthTiles, int drawHeightTiles, float rotationDegrees) const {
    ...
    if (!texture.isLoaded()) {
        DrawRectangleLines(static_cast<int>(x), static_cast<int>(y), static_cast<int>(w), static_cast<int>(h), DARKGRAY);
        DrawText("X", static_cast<int>(x + w * 0.35f), static_cast<int>(y + h * 0.25f), static_cast<int>(tileSize * 0.5f), DARKGRAY);
        return;
    }
