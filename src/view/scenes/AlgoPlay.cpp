...
void AlgoPlay::draw() {
    DrawText("PLAY WITH ALGORITHM", 210, 25, 30, BLACK);
    ...
    if (GuiButton(Rectangle{20, 200, 70, 36}, selectedAlgorithm == AlgorithmType::ASTAR ? ">A*" : "A*")) selectedAlgorithm = AlgorithmType::ASTAR;
    if (GuiButton(Rectangle{100, 200, 70, 36}, selectedAlgorithm == AlgorithmType::UCS ? ">UCS" : "UCS")) selectedAlgorithm = AlgorithmType::UCS;
    if (GuiButton(Rectangle{20, 245, 70, 36}, selectedAlgorithm == AlgorithmType::GBFS ? ">GBFS" : "GBFS")) selectedAlgorithm = AlgorithmType::GBFS;
    if (GuiButton(Rectangle{100, 245, 70, 36}, "Run")) runSolver();

    if (GuiButton(Rectangle{20, 300, 70, 36}, playing ? "Pause" : "Play")) playing = !playing;
    ...
    if (gameState.isBoardSelected()) {
        ...
        DrawText("No board selected", static_cast<int>(boardBounds.x), static_cast<int>(boardBounds.y), 24, RED);
    }

    DrawText(statusMessage.c_str(), 200, GetScreenHeight() - 58, 20, BLACK);
    if (gameState.isHasResult()) {
        SolveResult result = gameState.getResult();
        DrawText(TextFormat("Found: %s | Cost: %d | Moves: %s", result.isFound ? "Yes" : "No", result.cost, result.moves.c_str()),
                 200, GetScreenHeight() - 32, 18, BLACK);
        DrawText(TextFormat("Step: %d/%d | Time: %.3f ms | Iterations: %d", currentStep,
                 ...
                 200, GetScreenHeight() - 12, 16, DARKGRAY);
    }
}
